...
4. Почему без val-выборки невозможно диагностировать overfitting?
...
- **Underfitting** — модель не нашла закономерности даже в train; лечится усложнением модели или добавлением признаков, а не данных.
- **Overfitting** — модель выучила шум: train≈1.0, val≈0.7; лечится упрощением (`max_depth`, `min_samples_leaf`), регуляризацией или большим датасетом.
- **Good fit** — не «идеальное» равенство train и val, а разумный баланс: небольшой gap допустим, важно чтобы val был приемлем для задачи.
- **Разрыв train/val** — диагноз ставится именно по gap, а не по абсолютному значению: train=0.92, val=0.91 — отлично; train=0.99, val=0.72 — overfitting даже если val выглядит «нормально».