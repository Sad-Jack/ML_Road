...
- **XGBoost** — первый «промышленный» бустинг (2014): добавил L1/L2-регуляризацию прямо в функцию потерь. Параллелизм по признакам (не объектам); зрелая экосистема и интеграции. Категории — только через OHE вручную.
- **LightGBM** — leaf-wise рост дерева вместо level-wise: находит лист с максимальным снижением loss и разбивает только его. На 100k+ строках в 3–10 раз быстрее XGBoost при сопоставимом качестве; native categorical через `categorical_feature`.
- **CatBoost** — нативная обработка категорий через ordered target statistics (без leakage); единственный из трёх, который правильно обрабатывает категории без предварительного кодирования. Медленнее LightGBM, но часто лучший дефолт при наличии категорий.
