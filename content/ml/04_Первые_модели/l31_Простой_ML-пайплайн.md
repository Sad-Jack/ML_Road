...
2. В Pipeline три шага: `SimpleImputer → масштабирование признаков → LogisticRegression`. При вызове `pipe.fit(X_train, y_train)` какой из шагов вызывает `.fit_transform()`, а какой — просто `.fit()`? Почему последний шаг особенный?
