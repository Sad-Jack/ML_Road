...
4. Из урока 16 мы знаем, что test трогаем только один раз и нельзя подглядывать в него при настройке. Как Pipeline автоматически защищает от утечки данных при каждом fold `cross_val_score`?
...
- **Pipeline** — объект sklearn, объединяющий несколько шагов preprocessing и модель в одну цепочку.
- **ColumnTransformer** — sklearn-трансформер для применения разных шагов к разным колонкам.
- **fit_transform** — обучить трансформер на данных и сразу применить к ним.
- **transform** — применить уже обученный трансформер к новым данным (только transform, не fit).